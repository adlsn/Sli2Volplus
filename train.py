...
if __name__ == "__main__":
    try:
        ...
        if datatype == "C4KC_KiTS":
            ...
            headfolder = (
                r"/afs/crc.nd.edu/user/d/dan3/Dataset/SliData/C4KC-KiTS/C4KC-KiTS/"
            )

            subfolders = glob.glob(os.path.join(headfolder, "*", "*", "*"))

            folders_training = subfolders[0 : int(len(subfolders) * 0.9)]
            folders_validation = subfolders[
                int(len(subfolders) * 0.9) : len(subfolders)
            ]
        ...
        # elif datatype == "decathon_liver":   # slow to load
        #     '''
        #     Files-decathon_liver
        #     '''
        #     from dataset import Dataset_decathon_liver_vol as Dataset
        #
        ...
        #     # headfolder = r"/afs/crc.nd.edu/user/d/dan3/Dataset/SliData/Task03_Liver/imagesTr"
        #     headfolder = r"/afs/crc.nd.edu/user/d/dan3/Dataset/SliData/Task03_Liver/labelsTr"
        #     # headfolder = r"D:\data\Task03_Liver\Task03_Liver\labelsTr"
        ...
        #     # validation_generator = data.DataLoader(validation_set, **params, collate_fn=my_collate)
        ...
        elif datatype == "decathon_liver":
            """
            Files-decathon_liver
            """
            from dataset import Dataset_decathon_liver as Dataset

            # headfolder = '/run/media/hugoyeung/Data/Maddy/DecathLiver/processed_data/labelsTr/'
            # headfolder = '/run/media/hugoyeung/Data/Deep_Mind_Medical_Data/Task03_Liver/labelsTr/'
            # headfolder = r"/afs/crc.nd.edu/user/d/dan3/Dataset/SliData/Task03_Liver/imagesTr"
            headfolder = r"data/Task03_Liver_dcm/imagesTr"
            # headfolder = r"D:\data\Task03_Liver\Task03_Liver\labelsTr"
            subfolders = glob.glob(os.path.join(headfolder, "*"))
            ...
            """
            C4KC_KiTS, CT_Lymph_Nodes, Pancreas-CT
            """
            from dataset import Dataset_Tr as Dataset

            headfolder = r"data/final_data/TrainingSet/Task01_BrainTumour_dcm/imagesTr"
            subfolders = glob.glob(os.path.join(headfolder, "*"))

            folders_training = subfolders[0 : int(len(subfolders) * 1)]
            folders_validation = subfolders[
                int(len(subfolders) * 0.9) : len(subfolders)
            ]
        ...
        Save path
        """
        path_name = "PL_assisted_model_param/sli2vol_generate_pseudo_label_MRI"
        ...
        for epoch in range(max_epochs):
            ...
            for sub in range(0, len(folders_training), interval):
                ...
                training_set = Dataset(
                    sub_list, batch_size, set_size, mode="training_s1"
                )
